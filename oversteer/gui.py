...
class Gui:
    ...
    def populate_window(self):
        ...
        for device in self.device_manager.get_devices():
            devices.append([device.id, device.name + " " + device.dev_path])
    ...
    def load_preferences(self):
        ...
        if 'DEFAULT' in config:
            ...
            self.check_permissions = bool(int(config['DEFAULT'].get('check_permissions', 1)))
    ...
    def save_profile(self, profile_name, new = False):
        if not profile_name:
            return
        profile_file = os.path.join(self.app.profile_path, profile_name + '.ini')
        if os.path.exists(profile_file) and new:
