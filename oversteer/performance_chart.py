...
class PerformanceChart:
    ...
    def __init__(self, input_values, output_values, wheelrange):
        ...
        self.fposdata = self.posdata.filter(10)
    ...
    def get_latency(self):
        ...
        for t, v in posdata:
            if abs(v - v0) > noiselevel:
                return t - self.input.get_period(1)
    ...
    def get_time_to_max_accel(self):
        return max(self.facceldata.slice(*self.input.get_range(1, 2)), key = lambda i : abs(i[1]))[0] - self.input.get_period(1)

    def get_max_decel(self):
        t1 = self.input.get_period(2)
    ...
    def get_residual_decel(self):
        t1 = self.input.get_period(4)
        t2 = self.fveldata.xzero_time(*self.input.get_range(4, 5))
        if t2 is None:
            t2 = self.input.get_period(5)
        t2 = min(t1 + 0.2, t2)
